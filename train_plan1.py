...
from dataLoader.KITTI_dataset import load_train_data, load_test1_data
from kitti_image_model_plan1 import Model
...
def test1(model, args, save_path, epoch):
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader, 0):
            sat_map, left_camera_k, grd_left_imgs, gt_shift_u, gt_shift_v, gt_heading, grd_height = [item.to(device) for item in data[:-1]]
            pred_u, pred_v = model(sat_map, grd_left_imgs, grd_height.permute(0,3,1,2), left_camera_k, gt_heading=gt_heading, mode='test')
...
def train(model, lr, args, save_path):
    for epoch in range(args.epochs):
        ...
        for Loop, Data in enumerate(train_loader, 0):
            ...
            loss = model(sat_map, grd_left_imgs, grd_height.permute(0,3,1,2), left_camera_k, gt_shift_u, gt_shift_v, gt_heading, mode='train')
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        model.load_state_dict(torch.load(os.path.join(save_path, 'model_2.pth')))
