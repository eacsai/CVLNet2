...
class CrossHeightAttention(nn.Module):
    ...
    def forward(self, x, y, z):
        ...
        # Project with multiple heads
        q = self.to_q(x).reshape(B, M, 1, self.middle_dim) # [B, S^2, 1, middle_head]
        k = self.to_k(y).reshape(B, M, N, self.middle_dim) # [B, S^2, H, middle_head]

        # Dot product attention along cameras
        dot = self.scale * torch.matmul(q, k.transpose(-1, -2)).reshape(B, M, N, 1)  # [B, S^2, H, 1]
        dot = dot.softmax(dim=-2)

        # Combine values (image level features).
        a = torch.sum(dot * z, dim=-2) # [B, S^2, 1]
...
class CrossViewAttention(nn.Module):
    ...
    def __init__(self, blocks, dim, qkv_bias, norm=nn.LayerNorm) -> None:
        ...
        self.grd_embed = nn.Conv2d(3, 256, 1)
        self.sat_embed = nn.Conv2d(1, 256, 1)
        self.sat_position = BEVEmbedding(256, 64)
        
        # 1 3 h w
        image_plane = generate_grid(32, 128)
        image_plane[:, 0] *= 1024
        image_plane[:, 1] *= 256
    ...
    def forward(self, sat_embedding, grd_feat, grd_height, u, left_camera_k, valid_index):
        '''
        ...
        K_inv = torch.inverse(left_camera_k)
        ...
        grd_position_flat = K_inv @ pixel_flat
        grd_position = rearrange(grd_position_flat, 'b d (h w) -> b d h w', h=h, w=w)
        grd_position_embed = self.grd_embed(grd_position)

        query = sat_embedding + L2_norm(sat_position_embed) # B 256 S S
        # query = sat_embedding
        key_feat = grd_feat + L2_norm(grd_position_embed) # B 256 H W
        ...
        value_last = generate_y_for_attn(S, grd_height, u) # [B, S^2, H, 1]
        ...
        output = self.cross_attention_layers[2](query, key, value_last)
