...
import torch
from VGG import VGGUnet, L2_norm
import data_utils
...
class Model(nn.Module):
    ...
    def forward(self, sat_map, grd_img_left, grd_img_height, left_camera_k, gt_shift_u=None, gt_shift_v=None, gt_heading=None,
             mode='train'):
        '''
        Args:
        ...
        # vis
        # grd_project_img = to_pil_image(grd_img_left[0])
        # grd_project_img.save('grd_img_left.png')

        # sat_project_img = to_pil_image(sat_map[2])
        # sat_project_img.save('sat_map.png')


        # # 移除不必要的维度，得到形状为 [256, 1024]
        # height_map = grd_height.squeeze(0).squeeze(0)  # 现在形状为 [256, 1024]
        # # 归一化到 [0, 1]
        # # 使用 Matplotlib 进行可视化（使用彩色图）
        # plt.imshow(height_map.cpu(), cmap='viridis')  # 使用 'viridis' 映射显示颜色
        # plt.colorbar(label='Ground Height')
        # plt.title('Height Map Visualization with Viridis Colormap')
        # plt.savefig('height_grd_img.png')   
        # plt.close()   
        ...
        for level in range(len(sat_feat_list)):
            ...
            if level == 0:
                grd_height = F.interpolate(grd_img_height.detach(), size=(H, W), mode='bilinear', align_corners=False)
                ...
                grd_feat_reshaped = grd_feat.view(B, C, -1)  # (B, C, H*W)
                sat_feat_normalized = F.normalize(sat_feat_reshaped, p=2, dim=1, eps=1e-8)
                grd_feat_normalized = F.normalize(grd_feat_reshaped, p=2, dim=1, eps=1e-8)
                dot = torch.matmul(sat_feat_normalized.transpose(1, 2), grd_feat_normalized)
                temperature = 0.01
                dot = F.softmax(dot / temperature, dim=-1)
                pred_height = torch.matmul(dot, grd_height.view(B, 1, -1).permute(0,2,1)).view(B, 1, A, A)
            else:
