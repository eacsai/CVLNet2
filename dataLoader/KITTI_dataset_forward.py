...
left_forward_grd_dir = 'image_02/grd_forward_map'  # 'image_02\\data' #
...
class SatGrdDataset(Dataset):
    def __init__(self, root, file,
                 transform=None, shift_range_lat=20, shift_range_lon=20, rotation_range=0):
        self.root = root

        self.meter_per_pixel = data_utils.get_meter_per_pixel(scale=1)
        ...
        if transform != None:
            ...
            self.forward_image_transform = transform[2]
    ...
    def __getitem__(self, idx):
        ...
        grd_height_imgs = torch.tensor([])
        grd_depth_imgs = torch.tensor([])
        grd_forward_imgs = torch.tensor([])
        sat_height_imgs = torch.tensor([])
        image_no = file_name[38:]
        ...
        with open(oxts_file_name, 'r') as f:
            ...
            left_forward_img = os.path.join(self.root, self.pro_grdimage_dir, drive_dir, left_forward_grd_dir,
                                            image_no.lower())
            height_img_name = os.path.join(self.root, self.pro_grdimage_dir, drive_dir, left_height_grd_dir,
                                            image_no.lower().replace('.png', '_grd_height.pt'))
            sat_height = os.path.join(self.root, self.pro_grdimage_dir, drive_dir, sat_height_dir,
                                            image_no.lower().replace('.png', '_sat_height.pt'))
            ...
            # read forward image
            with Image.open(left_forward_img, 'r') as GrdImg:
                grd_img_left_forward = GrdImg.convert('RGB')
                if self.forward_image_transform is not None:
                    grd_img_left_forward = self.forward_image_transform(grd_img_left_forward)
            ...
            grd_forward_imgs = torch.cat([grd_forward_imgs, grd_img_left_forward.unsqueeze(0)], dim=0)
        ...
        sat_align_cam = sat_rot.transform(sat_rot.size, Image.AFFINE,
                                          (1, 0, data_utils.CameraGPS_shift_left[0] / self.meter_per_pixel,
                                           0, 1, data_utils.CameraGPS_shift_left[1] / self.meter_per_pixel),
        ...
        sat_map =TF.center_crop(sat_rand_shift_rand_rot, data_utils.SatMap_process_sidelength)
